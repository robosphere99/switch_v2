...
function hashToken(token2) {
  return crypto3.createHash("sha256").update(token2).digest("hex");
}
...
async function uploadAvatar(req, res) {
  ...
  const filename = path6.basename(req.file.filename || req.file.path);
...
function signBillToken(orderId) {
  const sig = crypto7.createHmac("sha256", SECRET).update(`bill:${orderId}`).digest("base64url").slice(0, 10);
  return `${orderId}.${sig}`;
}
function verifyBillToken(token2) {
  ...
  const expectedFull = crypto7.createHmac("sha256", SECRET).update(`bill:${orderId}`).digest("base64url");
...
var postFirmware = async (req, res) => {
  ...
  for (const dir of candidateDirs) {
    try {
      ...
      if (req.file.buffer) {
        fs9.writeFileSync(target, req.file.buffer);
        writeSuccess = true;
      }
    } catch (err) {
...
var postProductsIdMedia = async (req, res) => {
  ...
  const product = await prisma.product.findUnique({ where: { id: productId } });
  if (!product) throw new AppError("NOT_FOUND", "Product not found");
  const filename = path10.basename(req.file.filename || req.file.path);
...
var getApkStatus = async (_req, res) => {
  try {
    ...
    const candidatePaths2 = [
      path10.join(webPublicMobileAppDir, "SwitchNest_Latest.apk"),
      path10.join(mobileAppDir, "SwitchNest_Latest.apk")
    ];
...
var postApkUpload = async (req, res) => {
  try {
    ...
    const latestApkPath = path10.join(webPublicMobileAppDir, "SwitchNest_Latest.apk");
    const versionedApkPath = path10.join(webPublicMobileAppDir, `SwitchNest_v${cleanVersion}.apk`);
    fs9.copyFileSync(uploadedPath, latestApkPath);
    fs9.copyFileSync(uploadedPath, versionedApkPath);
    if (fs9.existsSync(mobileAppDir)) {
      try {
        fs9.copyFileSync(uploadedPath, path10.join(mobileAppDir, "SwitchNest_Latest.apk"));
        fs9.copyFileSync(uploadedPath, path10.join(mobileAppDir, `SwitchNest_v${cleanVersion}.apk`));
      } catch (e) {
...
function hashKey4(raw) {
  return crypto11.createHash("sha256").update(raw).digest("hex");
}
...
function createApp() {
  ...
  for (const dir of candidateFwDirs) {
    if (dir && fs14.existsSync(dir)) {
      app.use("/firmware", express2.static(dir));
    }
  }
  app.get("/firmware/:filename", (req, res, next) => {
    ...
    for (const dir of candidateFwDirs) {
      if (dir && fs14.existsSync(dir)) {
        const fullPath = path16.join(dir, filename);
  ...
  for (const dir of candidateUploadDirs) {
    if (dir && fs14.existsSync(dir)) {
      app.use(["/uploads", "/api/uploads"], express2.static(dir));
    }
  }
