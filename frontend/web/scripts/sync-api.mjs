// Web build ko apps/api me bhi copy karta hai — Plesk pe IIS static files
// wahan se serve karta hai (index.html + assets). Pull = pura update.
import { cpSync, mkdirSync, rmSync, existsSync, readFileSync, writeFileSync } from "node:fs";
import { fileURLToPath } from "node:url";
import { dirname, join } from "node:path";

const here = dirname(fileURLToPath(import.meta.url));
const webDist = join(here, "..", "dist");
const apiRoot = existsSync(join(here, "..", "..", "..", "backend", "api"))
  ? join(here, "..", "..", "..", "backend", "api")
  : (existsSync(join(here, "..", "..", "api")) ? join(here, "..", "..", "api") : join(here, "..", "..", "..", "backend", "api"));

if (existsSync(join(webDist, "index.html"))) {
  mkdirSync(apiRoot, { recursive: true });
  cpSync(join(webDist, "index.html"), join(apiRoot, "index.html"));
  rmSync(join(apiRoot, "assets"), { recursive: true, force: true });
  mkdirSync(join(apiRoot, "assets"), { recursive: true });
  cpSync(join(webDist, "assets"), join(apiRoot, "assets"), { recursive: true });

  // Add build cache buster to index.html so browsers never serve stale cached index.js
  let html = readFileSync(join(webDist, "index.html"), "utf8");
  const buildId = Date.now().toString(36);
  html = html.replace(/src="\/assets\/index\.js(\?[^"]*)?"/g, `src="/assets/index.js?v=${buildId}"`);
  html = html.replace(/href="\/assets\/index\.css(\?[^"]*)?"/g, `href="/assets/index.css?v=${buildId}"`);
  writeFileSync(join(webDist, "index.html"), html, "utf8");
  writeFileSync(join(apiRoot, "index.html"), html, "utf8");

  // Copy legacy hashes so cached browsers requesting old index-[hash].js files get index.js without 404
  const apiAssets = join(apiRoot, "assets");
  const mainJs = join(apiAssets, "index.js");
  if (existsSync(mainJs)) {
    const legacyHashes = ["index-Bc2l33nz.js", "index-Bc2133mz.js", "index-Df8JkqRD.js", "index-CK8FOSB3.js"];
    for (const name of legacyHashes) {
      cpSync(mainJs, join(apiAssets, name));
    }
  }
  const mainCss = join(apiAssets, "index.css");
  if (existsSync(mainCss)) {
    const legacyCssHashes = ["index-DcBVbXpn.css"];
    for (const name of legacyCssHashes) {
      cpSync(mainCss, join(apiAssets, name));
    }
  }
  console.log(`[sync-api] web build copied to apps/api with cache-buster v=${buildId} (index.html + assets + legacy hashes)`);
} else {
  console.warn("[sync-api] no dist/index.html found — skipping copy");
}
