...
import { View, Text, ScrollView, TouchableOpacity, Image, ActivityIndicator, Alert, StyleSheet, Modal, TextInput, KeyboardAvoidingView, Platform, PermissionsAndroid, Animated, Easing, Dimensions } from 'react-native';
...
export function ShopScreen() {
    ...
    const [shipping, setShipping] = useState({ name: '', phone: '', address: '', state: '', pincode: '' });
    ...
    const addToCart = (product: Product, event?: any) => {
        if (event && product.imageUrl) {
            ...
            const fullUri = product.imageUrl.startsWith('http') ? product.imageUrl : API_BASE + product.imageUrl;
            setFlyingItems(prev => [...prev, { id: newId, uri: fullUri, x: pageX - 30, y: pageY - 30 }]);
            setTimeout(() => {
    ...
    const doCheckout = async () => {
        if (!shipping.name || !shipping.phone || !shipping.address || !shipping.state || !shipping.pincode) {
            showAlert("Incomplete", "Please completely fill the shipping address, including state and pincode.");
            return;
        }
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.background }]}>
                    ...
                    <View style={styles.grid}>
                        {products.map((prod) => (
                            <TouchableOpacity key={prod.id} style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]} onPress={() => setSelectedProduct(prod)}>
                                {prod.imageUrl ? (
                                    <Image source={{ uri: prod.imageUrl.startsWith('http') ? prod.imageUrl : API_BASE + prod.imageUrl }} style={styles.img} resizeMode="contain" />
                                ) : (
                ...
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={[styles.modalBg, { backgroundColor: theme.background }]}>
                    ...
                    <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 24 }}>
                        {cart.map((c) => (
                            <View key={c.product.id} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderColor: theme.border }}>
                                <View style={{ flex: 1 }}>
                                    ...
                                    <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: theme.primary + '20', borderRadius: 8, overflow: 'hidden', alignSelf: 'flex-start', marginTop: 8 }}>
                                        <TouchableOpacity onPress={() => decreaseQty(c.product)} style={{ padding: 8 }}>
                                            <Minus color={theme.primary} size={14} />
                                        </TouchableOpacity>
                                        <Text style={{ color: theme.text, fontWeight: 'bold', paddingHorizontal: 12 }}>{c.qty}</Text>
                                        <TouchableOpacity onPress={(e) => addToCart(c.product, e)} style={{ padding: 8 }}>
                        ...
                        <View style={{ marginVertical: 24, padding: 16, backgroundColor: theme.card, borderRadius: 12, borderWidth: 1, borderColor: theme.border }}>
                            ...
                            <View style={{ flexDirection: 'row', gap: 12 }}>
                                <TextInput
                                    placeholder="State" placeholderTextColor={theme.textSecondary}
                                    value={shipping.state} onChangeText={t => setShipping(p => ({ ...p, state: t }))}
                                    style={[styles.formInput, { flex: 1, borderColor: theme.border, color: theme.text }]}
                                />
                                <TextInput
                                    placeholder="Pincode" placeholderTextColor={theme.textSecondary} keyboardType="number-pad"
                                    value={shipping.pincode} onChangeText={t => setShipping(p => ({ ...p, pincode: t }))}
                                    style={[styles.formInput, { flex: 1, borderColor: theme.border, color: theme.text }]}
                                />
                            </View>
                            <TextInput
            ...
            <RNModal 
                ...
                propagateSwipe
            >
                <View style={{ backgroundColor: theme.background, height: '90%', borderTopLeftRadius: 24, borderTopRightRadius: 24 }}>
                    <View style={styles.modalHeader}>
                    ...
                    {selectedProduct && (
                        <ScrollView style={{ flex: 1 }}>
                            <ProductSlider product={selectedProduct} />
                    ...
                    {selectedProduct && (
                        <View style={{ padding: 24, paddingBottom: Platform.OS === 'ios' ? 40 : 24, borderTopWidth: 1, borderColor: theme.border, backgroundColor: theme.background, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                            <View>
                                <Text style={{ color: theme.textSecondary, fontSize: 12 }}>Total Price</Text>
                                <Text style={{ color: theme.primary, fontSize: 24, fontWeight: 'bold' }}>₹{parseFloat(selectedProduct.price).toLocaleString('en-IN')}</Text>
                            </View>
                            {(() => {
                                const inCart = cart.find(c => c.product.id === selectedProduct.id);
                                if (inCart) {
                                    return (
                                        <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: theme.primary, borderRadius: 12, overflow: 'hidden', height: 48 }}>
                                            <TouchableOpacity onPress={() => decreaseQty(selectedProduct)} style={{ paddingHorizontal: 16, height: '100%', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.1)' }}>
                                                <Minus color="#000" size={18} />
                                            </TouchableOpacity>
                                            <Text style={{ color: '#000', fontWeight: 'bold', fontSize: 16, paddingHorizontal: 16 }}>{inCart.qty}</Text>
                                            <TouchableOpacity onPress={(e) => addToCart(selectedProduct, e)} style={{ paddingHorizontal: 16, height: '100%', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.1)' }}>
                                ...
                                return (
                                    <TouchableOpacity 
                                        onPress={(e) => addToCart(selectedProduct, e)} 
                                        disabled={(selectedProduct.stockCount || 0) <= 0} 
                                        style={[{ backgroundColor: (selectedProduct.stockCount || 0) > 0 ? theme.primary : theme.border, paddingHorizontal: 32, height: 48, borderRadius: 12, justifyContent: 'center', alignItems: 'center' }]}
                                    >
                                        <Text style={{ color: (selectedProduct.stockCount || 0) > 0 ? '#000' : theme.textSecondary, fontWeight: 'bold', fontSize: 16 }}>
                                            {(selectedProduct.stockCount || 0) > 0 ? 'Add to Cart' : 'Out of Stock'}
                                        </Text>
                                    </TouchableOpacity>
                                );
                            })()}
