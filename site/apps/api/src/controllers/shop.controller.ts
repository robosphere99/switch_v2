...
export async function getOrderStickers(req: Request, res: Response): Promise<void> {
...
export async function cancelShopOrder(req: Request, res: Response): Promise<void> {
  const id = Number(req.params.id);
...
export async function demoPayment(req: Request, res: Response): Promise<void> {
  const id = Number(req.params.id);
  const order = await prisma.order.findUnique({ where: { id } });
  if (!order || order.userId !== req.user!.sub) {
    throw new AppError("NOT_FOUND", "Order not found");
...
export async function getCurrentWifi(req: Request, res: Response): Promise<void> {
