...
async function run() {
  const user = await prisma.user.findUnique({ where: { username: 'jhatu' }, include: { ownedHomes: true } });
  if (!user) {
    console.log('User jhatu not found');
    process.exit(0);
  }
  ...
  try {
      ...
      console.log('User jhatu deleted successfully.');
  } catch (err) {
      console.error('Failed to delete jhatu:', err);
  }
  process.exit(0);
}
