...
| **E008** | `HTTP 404 on /api/* & /socket.io/*` under IIS | `web.config` / `app.js` | IIS served static files but returned native 404 for dynamic endpoints without URL rewrite rule. | Used clean `<rewrite>` rule in `web.config` forwarding non-files to `app.js` without duplicate `<handlers>` tags. | Never re-declare `<handlers>` in `web.config`; use clean `<rewrite>` rules. |
